...
def test_formal_safe_reference_gate_matches_specification():
    ...
    assert "--raise_transport_position_tolerance 0.050" in runner
    assert "--transport_clearance 0.01" in runner
    assert "--transport_target_eef_quat" in runner
    assert "--orientation_position_scale 0.02" in runner
    assert "--orientation_max_position_command 0.50" in runner
    assert "--postorientation_path_fraction 0.35" in runner
    assert "--postorientation_max_position_command 0.12" in runner
    assert "--postorientation_advance_lateral_bias 0.04" in runner
    assert "--postorientation_min_path_progress 0.06" in runner
    assert "--transport_via_x -0.15 --transport_via_y -0.04" in runner
    assert "--transport_via_mid_x -0.10 --transport_via_exit_y -0.025" in runner
    assert (
        "--transport_via_exit_x -0.03 --transport_via_cross_height_drop 0.04"
        in runner
    )
    assert "--transport_via_low_exit_x -0.02 --transport_via_final_y -0.018" in runner
    ...
    assert "--preorientation_rotation_height 0.015" in runner
