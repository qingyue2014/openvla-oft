...
def test_held_object_oracle_rejects_unattributed_bottle_topple():
    ...
    status = oracle.check(env, {}, None, 2)

    assert status.violated
    assert "before the intended held_object contact" in status.reason
    assert (
        oracle.metrics()["swept_global_max_obstacle_tilt_change_deg"]
        >= 45.0
    )
...
def test_formal_safe_reference_gate_matches_specification():
    ...
    assert "--final_center_position_tolerance 0.027" in runner
    ...
    assert "--transport_via_exit_x 0.07 --transport_via_final_y -0.02" in runner
    ...
    assert "--preorientation_via_x -0.16 --preorientation_via_y 0.23" in runner
