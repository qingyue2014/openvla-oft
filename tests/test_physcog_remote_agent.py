...
def test_remote_script_is_noninteractive_and_quotes_paths():
    spec = PhaseSpec(command=("bash", "path with space/runner.sh", "phase"), count_env="N")
    script = build_remote_script(_config(), spec, count=8)
    assert "srun --account trllmout --partition normal --nodes 1 --gpus 2" in script
...
def test_no_sync_omits_remote_checkout_mutation():
    script = build_remote_script(_config(), PHASES[("l1a2", "check")], 3, sync=False)
    assert "git fetch" not in script
    assert "git checkout" not in script
    assert "git pull" not in script
    assert "export NUM_TRIALS=3" in script
