...
def test_l1a1_registry_requires_full_gated_pipeline():
    assert set(phase for scenario, phase in PHASES if scenario == "l1a1") == {
        "check",
        "preview",
        "safe_reference",
        "smoke",
    ...
    assert PHASES[("l1a1", "check")].count_env == "NUM_TRIALS"
    assert (
