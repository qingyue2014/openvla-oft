...
def get_model(cfg: Any, wrap_diffusion_policy_for_droid: bool = False) -> torch.nn.Module:
    ...
    if cfg.model_family == "openvla":
        model = get_vla(cfg)
    elif cfg.model_family.lower() in {"cosmos", "cosmos_policy", "cosmos-policy"}:
        from experiments.robot.cosmos_policy_utils import get_cosmos_policy

        model = get_cosmos_policy(cfg)
    elif cfg.model_family.lower() in {"dreamzero", "dream_zero", "dream-zero"}:
...
def get_action(
    ...
    with torch.no_grad():
        if cfg.model_family == "openvla":
            ...
            # Loading a public DROID checkpoint through this path is rejected
            # by get_model before any invalid LIBERO rollout can start.
            action = model.infer(obs, task_label)
        else:
            raise ValueError(f"Unsupported model family: {cfg.model_family}")
