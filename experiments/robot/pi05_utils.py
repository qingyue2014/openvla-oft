...
The model runs in OpenPI's own environment and is queried through the official
``openpi-client`` websocket protocol.  Keeping inference in a separate process
avoids dependency conflicts with this repository's OpenVLA runtime.
"""
...
def resize_with_pad(
    ...
    """Match OpenPI's LIBERO aspect-preserving resize with zero padding."""
...
def get_pi05_policy(cfg: Any) -> Any:
    """Connect to an official OpenPI policy server hosting ``pi05_libero``."""
    try:
...
def get_pi05_action(
    ...
    """Query OpenPI and validate its direct LIBERO action chunk."""
    request = {
