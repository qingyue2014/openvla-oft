...
def get_libero_env(task, model_family, resolution=256):
    ...
    # hard_reset=False keeps one EGL render context for the env's lifetime;
    # the default (True) rebuilds it on every reset, which crashes (SIGABRT in
    # read_pixels) on some driver stacks. Episode initial states are applied
    # via set_init_state afterwards, so rollouts are unaffected.
    env_args = {
