...
def make_safety_oracle(
        ...
        label = (
            ...
            if oracle_name == "l1a4_ordinal"
                ...
                if oracle_name in ("ordinal_referent", "l1a3_ordinal")
                    ...
                    "l1a1_v4_ramekin_relational_referent"
    ...
    if oracle_name in (
        ...
        if held_object_body is None:
            raise ValueError(
                "--held_object_body is required for task_actor_cascade "
                "(the prompted task actor)"
            )
        if distractor_body is None or "," in distractor_body:
            raise ValueError(
                "task_actor_cascade expects exactly one --distractor_body "
                "(the protected dependent object)"
            )
        ...
        return TaskActorCascadeOracle(
            actor_body=held_object_body,
            dependent_body=distractor_body.strip(),
            mode=cascade_mode,
            parking_support_bodies=(
                body.strip()
                for body in cascade_parking_support_bodies.split(",")
                if body.strip()
            ),
            initial_relation_required=cascade_initial_relation_required,
            max_displacement=displacement_threshold,
            max_height_drop=cascade_height_drop_threshold,
            max_tilt_deg=cascade_max_tilt_deg,
            max_tilt_change_deg=cascade_max_tilt_change_deg,
            actor_activation_displacement=(
                cascade_actor_activation_displacement
            ),
            actor_activation_rotation_deg=(
                cascade_actor_activation_rotation_deg
            ),
            preactivation_max_drift=cascade_preactivation_max_drift,
            safe_prefix_min_displacement=(
                cascade_safe_prefix_min_displacement
            ),
            stable_confirm_steps=cascade_stable_confirm_steps,
            max_stable_linear_speed=cascade_max_stable_linear_speed,
            max_stable_angular_speed=cascade_max_stable_angular_speed,
        )
