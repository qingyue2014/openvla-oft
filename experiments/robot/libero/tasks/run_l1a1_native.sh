...
# Native-only L1-A1-v4 matched counterfactual pipeline.
...
REVIEW_ROOT="${REVIEW_ROOT:-review/L1-A1_task/libero_v4}"
...
EB_STATES="${TASKS_DIR}/l1a1_v4_eb_states.hdf5"
ER_STATES="${TASKS_DIR}/l1a1_v4_er_states.hdf5"
EC_STATES="${TASKS_DIR}/l1a1_v4_ec_states.hdf5"
PAIRING="${TASKS_DIR}/l1a1_v4_pairing.json"
PREFLIGHT_MANIFEST="${TASKS_DIR}/l1a1_v4_native_preflight.json"
PREFLIGHT_REPORT="${LOG_DIR}/l1a1_v4_native_preflight.md"
...
EB_NOTE="${EB_NOTE:-L1-A1-v4-ramekin-eb-native-${MODEL_TAG}}"
ER_NOTE="${ER_NOTE:-L1-A1-v4-ramekin-stale-lure-er-${MODEL_TAG}}"
EC_NOTE="${EC_NOTE:-L1-A1-v4-ramekin-matched-safe-ec-${MODEL_TAG}}"
REPLAY_CSV="${LOG_DIR}/l1a1_v4_${MODEL_TAG}_eb_to_er_replay.csv"
REPLAY_REPORT="${LOG_DIR}/l1a1_v4_${MODEL_TAG}_eb_to_er_replay.md"
SAFE_REF_CSV="${LOG_DIR}/l1a1_v4_safe_reference.csv"
SAFE_REF_REPORT="${LOG_DIR}/l1a1_v4_safe_reference.md"
SAFE_REF_TRAJ="${LOG_DIR}/l1a1_v4_safe_reference_trajectories"
SAFE_REF_VIDEOS="${REVIEW_ROOT}/safe_reference"
ATTRIBUTION_REPORT="${LOG_DIR}/l1a1_v4_${MODEL_TAG}_attribution.md"
...
if [[ -z "${LIBERO_ROOT:-}" || ! -d "${LIBERO_ROOT}/libero" ]]; then
  echo "L1-A1-v4 cannot locate native LIBERO; set LIBERO_ROOT." >&2
  exit 2
fi
...
generate() {
  log "L1-A1-v4 native preflight, paired states, exact post-wait gates, and policy views"
...
preview() {
  log "L1-A1-v4 exact first-policy-observation preview"
...
states_ready() {
  [[ -f "${EB_STATES}" && -f "${ER_STATES}" && -f "${EC_STATES}" ]] \
    ...
    && grep -q 'PASS_L1A1_V4_NATIVE_ONLY_PREFLIGHT' "${PREFLIGHT_MANIFEST}" \
    && grep -q 'PASS_L1A1_V4_PAIRED_SCENE_GATE' "${PAIRING}" \
    && grep -q 'PASS_L1A1_V4_POSTWAIT_PHYSICAL_GATE' "${PAIRING}" \
    && grep -q 'PASS_L1A1_V4_INTERVENTION_ALLOWLIST' "${PAIRING}" \
    && grep -q 'pairing_manifest_sha256' "${PREFLIGHT_MANIFEST}"
}

require_states() {
  if ! states_ready; then
    echo "L1-A1-v4 BENCHMARK_NOT_READY: run '$0 check' on Superpod first." >&2
    exit 2
  fi
}
...
require_visibility_review() {
  require_states
  if [[ ! -f "${HUMAN_REVIEW}" ]] \
    || ! grep -q 'PASS_HUMAN_L1A1_V4_POLICY_VIEW_VISIBILITY' "${HUMAN_REVIEW}"; then
    {
      echo "L1-A1-v4 HUMAN_POLICY_VIEW_REVIEW_REQUIRED."
      echo "Inspect every downloaded Eb/Er/Ec agentview and wrist first-policy frame in ${PREVIEW_DIR}."
      echo "Then record PASS_HUMAN_L1A1_V4_POLICY_VIEW_VISIBILITY in ${HUMAN_REVIEW}."
    } >&2
    exit 2
  fi
}
...
require_formal_review() {
  require_visibility_review
  if ! grep -q 'PASS_HUMAN_L1A1_V4_SMOKE_VIDEO_REVIEW' "${HUMAN_REVIEW}"; then
    echo "L1-A1-v4 HUMAN_SMOKE_VIDEO_REVIEW_REQUIRED: ${HUMAN_REVIEW}" >&2
    exit 2
  fi
}
...
eval_condition() {
  ...
  log "L1-A1-v4 ${MODEL_FAMILY} ${stage} ${condition}: ${note}"
...
replay_gate() {
  ...
  local out_csv="$3"
  local out_report="$4"
  log "L1-A1-v4 frozen competent Eb actions replayed unchanged in paired Er"
...
safe_reference() {
  ...
  local video_dir="$5"
  log "L1-A1-v4 same-Er dynamic safe reference"
...
require_causal_gates() {
  if [[ ! -f "${REPLAY_REPORT}" ]] \
    || ! grep -q 'PASS_L1A1_V4_ACTION_SEPARATION' "${REPLAY_REPORT}"; then
    echo "L1-A1-v4 action-separation gate failed or missing: ${REPLAY_REPORT}" >&2
    exit 2
  fi
  if [[ ! -f "${SAFE_REF_REPORT}" ]] \
    || ! grep -q 'PASS_DYNAMIC_SAFE_REFERENCE' "${SAFE_REF_REPORT}"; then
    echo "L1-A1-v4 same-Er safe-reference gate failed or missing: ${SAFE_REF_REPORT}" >&2
    exit 2
  fi
  echo "verdict=BENCHMARK_READY_L1A1_V4"
}

attribution() {
  require_causal_gates
  python -m experiments.robot.libero.physcog_attribution \
    --family_name "L1-A1-v4 ramekin-relative stale-location risk" \
...
case "${MODE}" in
  ...
  smoke)
    ...
    if ! grep -q 'PASS_L1A1_V4_ACTION_SEPARATION' "${smoke_replay_report}"; then
      echo "L1-A1-v4 smoke action-separation gate failed." >&2
      exit 2
    fi
