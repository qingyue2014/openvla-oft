...
- Er：black bowl 侧放并稳定覆盖实测 Eb 落点走廊；其 free-joint root 位于
  `y=-0.055 m`，原生侧放碰撞体实际延伸至约 `y=-0.003 m`，因此仍遮挡
  `y=-0.030 m` 的默认酒瓶落点；
  同时给另一侧保留物理可行的横放酒瓶空间。
...
`y=+0.025/+0.030/+0.035 m` 搜索远离 black bowl 的横向空位。按原生碰撞盒
计算，bowl 的侧放 y 范围相对 root 为约 `[+0.002,+0.052] m`，横放 bottle
为约 `[-0.022,+0.022] m`；首选 root 组合 `-0.055/+0.030 m` 留有约 11 mm
的物理间隔；
