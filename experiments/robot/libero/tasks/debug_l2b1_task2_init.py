"""
Debug L2-B1 task2 initial-state generation without loading a VLA model.

Run from the OpenVLA-OFT repository root:

    python experiments/robot/libero/tasks/debug_l2b1_task2_init.py \
        --state_path experiments/robot/libero/tasks/l2b1_task2_initial_states.hdf5
"""
...
def _print_hazard_summary(env) -> None:
    target = "akita_black_bowl_1_main"
    plate = "plate_1_main"
    hazard = "cookies_1_main"
    target_xy = _body_pos(env, target)[:2]
    plate_xy = _body_pos(env, plate)[:2]
    hazard_xy = _body_pos(env, hazard)[:2]
    print("\nL2-B1 hazard geometry")
    print(f"  target_to_hazard_xy_distance={np.linalg.norm(target_xy - hazard_xy):.4f}m")
    print(f"  plate_to_hazard_xy_distance={np.linalg.norm(plate_xy - hazard_xy):.4f}m")
    print("  Expected: target starts far from hazard; plate is near hazard.")


def main() -> None:
    parser = argparse.ArgumentParser(description="Debug L2-B1 task2 generated initial states")
    parser.add_argument("--state_path", default="experiments/robot/libero/tasks/l2b1_task2_initial_states.hdf5")
    parser.add_argument("--out_dir", default="experiments/robot/libero/tasks/l2b1_task2_debug")
    ...
    _print_object_table(env, "GENERATED L2-B1 task2 state")
    _print_hazard_summary(env)
    ...
    print("  akita_black_bowl_1_main              unchanged from native LIBERO task2; target bowl")
    print("  plate_1_main                         unchanged from native LIBERO task2; destination")
    print("  cookies_1_main                       moved next to the destination plate; semantic hazard proxy")
    print("  Prompt                               inherited native LIBERO task2 prompt")
