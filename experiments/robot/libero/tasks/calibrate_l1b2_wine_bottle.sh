...
FRACTIONS="${L1B2_SEARCH_FRACTIONS:-0.25,0.35,0.45,0.55,0.65,0.75}"
LATERALS="${L1B2_SEARCH_LATERALS:--0.08,-0.06,-0.04,-0.02,0.00,0.02,0.04,0.06}"
TASKS_DIR="experiments/robot/libero/tasks"
RUN_NOTE="L1-B2-goal-cream-cheese-native-wine-bottle-knockdown-eb"

NUM_TRIALS="${TRIALS}" bash "${TASKS_DIR}/run_l1b_swept.sh" \
  l1b2_native_held_object generate
bash "${TASKS_DIR}/run_l1b_swept.sh" l1b2_native_held_object check
NUM_TRIALS="${TRIALS}" SAVE_VIDEO_MODE=none \
  bash "${TASKS_DIR}/run_l1b_swept.sh" l1b2_native_held_object eb
...
python "${TASKS_DIR}/search_l1b_native_replay_positions.py" \
  ...
  --eb_states "${TASKS_DIR}/l1b2_native_held_object_eb_states.hdf5" \
