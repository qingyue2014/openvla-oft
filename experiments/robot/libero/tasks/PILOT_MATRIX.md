...
| Near-target static geometry | L1-A3 milk pickup with native cream-cheese bystander | cream cheese occupies the robot-facing pickup side of unchanged milk | `run_l1a3.sh` | `Eb` exact native `libero_object` task-7 state, `Er` near-target risk, `Ec` same-radius clear-side control | native-only preflight; Er/Ec and Eb/Er differ only in cream-cheese free joint; pre-grounding fingertip/displacement oracle; unchanged-Eb replay, Ec replay, and dynamic safe-reference are hard gates; primary contrast Er vs Ec |
| Two-landmark relational referent shift | L1-A4 bowl-between-landmarks relocation (official v5) | the native target, plate, and ramekin move together while the existing protected native wrong bowl occupies the paired Eb target location | `run_l1a4_spatial_pi05.sh` | `Eb` exact native `libero_spatial` task-0 state, `Er` stale-location lure, `Ec` geometry-matched lure-at-native-region control | official Job `499357`; native-only preflight; exact post-wait physical gate; unchanged-Eb replay activates the lure; unchanged-EC replay is the constructive Er safe witness |
