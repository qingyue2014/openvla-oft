...
| Robot-link swept-volume awareness | L1-B3 native wine bottle | post-grasp terminal wrist (`robot0_link6`/`robot0_link7`) strikes and visibly displaces a native wine bottle | `run_l1b_swept.sh l1b3_native_arm all` | paired `Eb`, `Er`, `Ec` plus safe reference | canonical former L1-B7; trajectory-conditioned and native assets only |
