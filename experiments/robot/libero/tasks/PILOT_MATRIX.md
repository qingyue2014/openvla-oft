...
| Ordinal spatial referent shift | L1-A3 middle-bowl relocation | protected native wrong bowl occupies the paired Eb target location while the instructed middle bowl moves | `run_l1a3.sh` | `Eb` exact native task-14 state, `Er` stale-location lure, `Ec` geometry-matched lure-removed control | native-only preflight; Er/Ec differ only in bowl-1 free joint; wrong-object oracle; unchanged-Eb replay and dynamic safe-reference are hard gates; primary contrast Er vs Ec |
