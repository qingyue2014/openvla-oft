"""Fail-closed native-task and asset-closure contract for L1-A1 v2.
...
VERDICT = "PASS_L1A1_NATIVE_ONLY_PREFLIGHT"
INTERVENTION_ID = "l1a1_native_ramekin_relation_postwait_v2"
PHYSICAL_GATE_VERDICT = "PASS_L1A1_POSTWAIT_PHYSICAL_GATE"
...
def write_preflight(manifest_path: Path, report_path: Path) -> dict[str, object]:
    ...
    report_path.write_text(
        "\n".join(
            (
                "# L1-A1 v2 Native-Only Preflight",
                "",
                f"- Verdict: **{VERDICT}**",
...
def bind_generated_artifacts(
    ...
    if record.get("verdict") != VERDICT:
        raise ValueError("cannot bind artifacts to an invalid L1-A1 preflight")
...
def verify_evaluation_request(
    ...
    *,
    ...
    if record.get("verdict") != VERDICT:
        raise ValueError(f"invalid L1-A1 preflight verdict in {path}")
    ...
    for key in (
        ...
        if record.get(key) != fresh.get(key):
            raise ValueError(f"stale or mismatched L1-A1 preflight field: {key}")
    if (task_suite_name, int(task_id), task_language, policy_prompt) != (
        ...
        raise ValueError("L1-A1 runtime task or prompt mismatch")
    ...
    if len(matched) != 1 or matched[0].get("sha256") != _file_sha256(states):
        raise ValueError("L1-A1 runtime state artifact is not bound to the preflight")
    pairing = Path(record["pairing_manifest"]).resolve(strict=True)
    if _file_sha256(pairing) != record.get("pairing_manifest_sha256"):
        raise ValueError("L1-A1 pairing manifest changed after preflight binding")
...
def verify_runtime_asset_inventory(manifest_path: str | Path, model) -> dict[str, int]:
    ...
    if record.get("asset_inventory_sha256") != _json_sha256(
        ...
        raise ValueError("L1-A1 native asset inventory signature mismatch")
    resolved: dict[str, int] = {}
    for body in EXPECTED_RUNTIME_BODIES:
        try:
            resolved[body] = int(model.body_name2id(body))
        except Exception as exc:
            raise ValueError(f"L1-A1 native body missing at runtime: {body}") from exc
    return resolved
...
def main() -> None:
    ...
    parser.add_argument(
        "--manifest",
        default="experiments/robot/libero/tasks/l1a1_v2_native_preflight.json",
    )
    parser.add_argument(
        "--report", default="experiments/logs/l1a1_v2_native_preflight.md"
    )
