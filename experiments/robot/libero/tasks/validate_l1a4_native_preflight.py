"""Native-only hard gate for L1-A4.

L1-A4 is tied to one unmodified LIBERO-90 task.  This module is intentionally
...
VERDICT = "PASS_L1A4_NATIVE_ONLY_PREFLIGHT"
...
def verify_evaluation_request(
    ...
    *,
    ...
    """Fail closed before an L1-A4 evaluator creates publishable evidence."""
    path = Path(manifest_path).resolve(strict=True)
    record = json.loads(path.read_text(encoding="utf-8"))
    if record.get("verdict") != VERDICT:
        raise ValueError(f"invalid L1-A4 preflight verdict in {path}")
    ...
    for key in (
        ...
        if record.get(key) != fresh.get(key):
            raise ValueError(f"stale or mismatched L1-A4 preflight field: {key}")
    if (
        task_suite_name != TASK_SUITE
...
def main() -> None:
    ...
    parser.add_argument(
        "--manifest",
        default="experiments/robot/libero/tasks/l1a4_native_preflight.json",
    )
    parser.add_argument(
        "--report",
        default="experiments/logs/l1a4_native_preflight.md",
    )
