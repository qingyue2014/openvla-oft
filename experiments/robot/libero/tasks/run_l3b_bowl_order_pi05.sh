...
SERVER_LOG="${SERVER_LOG:-experiments/logs/l3b_bowl_pi05_server.log}"
RUNTIME_CACHE_ROOT="${RUNTIME_CACHE_ROOT:-${TMPDIR:-/tmp}/l3b-bowl-${SLURM_JOB_ID:-local}}"

if [[ "${MODE}" == "prepare" || "${MODE}" == "check" || "${MODE}" == "summarize" ]]; then
  exec bash experiments/robot/libero/tasks/run_l3b_bowl_order.sh "${MODE}"
fi

if [[ "${MODE}" == "complete_smoke" ]]; then
  bash experiments/robot/libero/tasks/run_l3b_bowl_order.sh prepare
  MODE="smoke"
fi
...
mkdir -p "$(dirname "${SERVER_LOG}")" "${RUNTIME_CACHE_ROOT}/numba" "${RUNTIME_CACHE_ROOT}/triton"
...
(
  ...
  CUDA_VISIBLE_DEVICES="${PI05_SERVER_GPU}" \
    ...
    "${SERVER_PYTHON}" scripts/serve_policy.py --env LIBERO --port "${PI05_PORT}"
) >"${SERVER_LOG}" 2>&1 &
server_pid=$!
...
bash experiments/robot/libero/tasks/run_l3b_bowl_order.sh "${MODE}"
