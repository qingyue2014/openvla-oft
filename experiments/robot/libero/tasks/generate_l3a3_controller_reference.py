...
def _bounded_contact_seek_vertical_stabilization_action(
    *,
    ...
    if not (
        ...
        and 0.0 < maximum_translation_action <= 1.0
        and np.isfinite(progress_resolution_m)
    ...
    available_downward_world_step = float(
        live_table_clearance - required_post_action_clearance
    )
...
def _fixed_safe_z_lateral_hold_action(
    *,
    ...
    requested_outward_component = float(
        np.dot(requested_xy_action, outward_direction_xy)
    )
    requested_xy_norm = float(np.linalg.norm(requested_xy_action))
    lateral_error_m = float(
        np.linalg.norm(lateral_target_xy - current_eef[:2])
    )
    lateral_target_reached = bool(
        lateral_error_m <= lateral_position_tolerance_m
    )
    ...
    coupled_xy_neutralization_position_error_m = float(
        fixed_safe_z_m - current_eef[2]
    )
    coupled_xy_neutralization_projected_overshoot_m = float(
        max(0.0, -coupled_xy_neutralization_position_error_m)
        + max(0.0, measured_vertical_step_progress_m)
    )
    coupled_xy_neutralization_height_tracking_accepted = bool(
        abs(coupled_xy_neutralization_position_error_m)
        <= vertical_position_tolerance_m
        or (
            coupled_xy_neutralization_position_error_m
            < -vertical_position_tolerance_m
            and coupled_xy_neutralization_projected_overshoot_m
            <= 3.0 * vertical_position_tolerance_m
        )
    )
    coupled_xy_neutralization_in_progress = bool(
        release_slew_enabled
    ...
    coupled_xy_dynamic_lateral_hold_tolerance_m = float(
        lateral_position_tolerance_m
        + closed_loop_hazard_response_bound_m
    )
    coupled_xy_lateral_return_requests_inward = bool(
        requested_outward_component < 0.0
    )
    coupled_xy_full_outward_settle_hold_requested = bool(
        ...
        and projected_lateral_error_after_measured_outward_response_m
        <= coupled_xy_dynamic_lateral_hold_tolerance_m
        and coupled_xy_lateral_return_requests_inward
        and live_outside_clearance > outside_recovery_exit_clearance
        and outside_response_projected_clearance_m
        > outside_recovery_exit_clearance
        and live_table_clearance > outside_recovery_exit_clearance
    ...
    coupled_xy_transient_lateral_decrement_requested = bool(
    ...
    coupled_xy_neutralization_requested = bool(
        ...
        and not strict_target_axis_transition_repeat_wait_requested
        and (
            (
                lateral_target_reached
                and coupled_xy_neutralization_step_stable
    ...
    captured_outward_response_tracking_active_ceiling_m = float(
        ...
        else captured_outward_response_tracking_ceiling_m
    )
    ...
    if captured_outward_response_tracking_requested:
        ...
        captured_outward_response_unbounded_action_correction = float(
        ...
        strict_target_coupled_hold_refill_action_step_clipped = bool(
            strict_target_coupled_hold_refill_tracking_eligible
            and captured_outward_response_action_correction
            < captured_outward_response_unbounded_action_correction
        )
    ...
    measured_downward_tail = downward_vertical_response
    ...
    vertical_stability_confirmation_hold_eligible = bool(
        ...
        and np.linalg.norm(previous_commanded_action_xyz[:2])
        <= neutral_z_confirmation_xy_action_bound
    )
    if (
    ...
    return action, {
        ...
        "current_eef": current_eef.tolist(),
        "lateral_target_xy": lateral_target_xy.tolist(),
        "lateral_position_tolerance_m": float(
            lateral_position_tolerance_m
        ),
        ...
        "severe_vertical_response": severe_vertical_response,
        "required_outward_recovery_action": (
            required_outward_recovery_action
            if outside_recovery_active
            else None
        ...
        "strict_target_refill_transition_history_active": (
            strict_target_refill_transition_history_active
        ),
        ...
        "coupled_xy_lateral_return_requests_inward": (
            coupled_xy_lateral_return_requests_inward
        ),
        ...
        "coupled_xy_neutralization_projected_overshoot_m": (
            coupled_xy_neutralization_projected_overshoot_m
        ),
        "coupled_xy_neutralization_action": (
            coupled_xy_neutralization_action
        ),
        "pre_coupled_xy_neutralization_xy_action": (
            pre_coupled_xy_neutralization_xy_action.tolist()
        ),
        "outside_response_projected_clearance_m": (
            outside_response_projected_clearance_m
        ),
...
def _compiled_hazard_release_zero_coast_action(
    *,
    ...
    """Hold zero motion after a fully authorized hazard release."""
    try:
        native_low = np.asarray(native_action_spec["low"], dtype=float)
        native_high = np.asarray(native_action_spec["high"], dtype=float)
        native_source = str(native_action_spec["source"])
    ...
    return action, {
        "formula": (
            "after the dedicated two-frame hazard release, command zero "
            "XYZ and rotation while both outside and table clearances remain "
            "strictly above the unchanged recovery-exit reserve; require "
            "live post-action guards and absolute-response confirmation"
        ),
        "native_action_spec_source": native_source,
        "commanded_xyz_action": action[:3].tolist(),
        "commanded_rotation_action": action[3:6].tolist(),
        ...
        "guard_violations": list(
            outside_side_guard.get("violations", ())
        ),
        "proof": {
            "zero_translation_and_rotation": True,
            "strictly_inside_native_action_bounds": True,
            "outside_recovery_exit_reserve_preaccepted": True,
            "table_recovery_exit_reserve_preaccepted": True,
            "post_action_live_guards_required": True,
    ...
    structural_seek_context.update(
        {
            ...
            "vertical_corridor_hazard_release_zero_coast": (
                "after a dedicated hazard release with the full side guard "
                "and recovery-exit reserves accepted, command zero XYZ and "
                "rotation until two unchanged 0.050 mm absolute-response "
                "confirmation frames; stable above-rim coverage gaps return "
                "to the shielded descent"
            ),
        }
    )
    ...
    for guard_step in range(1, effective_structural_waypoint_budget + 1):
        ...
        if structural_stage == "fixed_safe_z_lateral_approach":
            ...
            fixed_safe_z_instantaneous_stable = bool(
                ...
                <= 2.0 * structural_max_translation_action
            )
        ...
        if structural_stage in {
            ...
            vertical_corridor_control_target[:2] = (
                ...
                else active_vertical_corridor_envelope[
                    "balanced_hold_target_xy"
                ]
            )
        ...
        if stage_before_action == "overhead_high_corridor_lateral":
            ...
            if hazard_release_zero_coast_active_before_action:
                zero_coast_response = lateral_settle_progress[
                    "step_response"
                ]
                zero_coast_instantaneous_stable = bool(
                    all(
                        abs(float(zero_coast_response[key]))
                        <= float(
                            args.minimum_saturated_waypoint_progress
                        )
                        for key in (
                ...
                zero_coast_stable_count = int(
                    lateral_settle_state.get(
                        "hazard_release_zero_coast_stable_count", 0
                    )
                    + 1
                    if zero_coast_instantaneous_stable
                    else 0
                )
            else:
            ...
            lateral_settle_progress[
                "hazard_release_zero_coast_stable_count"
            ] = zero_coast_stable_count
            lateral_settle_progress[
                "hazard_release_zero_coast_required_stable_count"
            ] = 2
            ...
            if lateral_settle_progress["settled"]:
                ...
                lateral_settle_state = None
                structural_stage = "vertical_corridor_descent"
                vertical_corridor_hazard_release_descent_active = True
                feedback["hazard_release_zero_coast_handoff"] = {
                    "event": (
                        "stable_zero_coast_above_rim_to_shielded_descent"
                    ),
                    "stable_count": zero_coast_stable_count,
                    "required_stable_count": 2,
                    "above_rim_handoff_evidence": (
                ...
                not hazard_release_zero_coast_active_before_action
                and
                ...
                vertical_tail_events.append(
                    {
                        "guard_step": int(guard_step),
                        **feedback[
                            "settle_geometric_authority_release"
                        ],
                    }
                )
            else:
