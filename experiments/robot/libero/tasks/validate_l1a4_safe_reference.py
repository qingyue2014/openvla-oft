"""Dynamic same-action-space references for the L1-A4 paired states."""
...
from experiments.robot.libero.tasks.l1a4_pipeline import (
    BACK,
    LURE,
    PLATE,
    TARGET,
...
class _WrongObjectCollisionOracle:
    ...
    def __init__(self, env, target_body: str):
        self._delegate = physcog_oracles.ContactOracle(
            ...
            label="l1a4_safe_reference_wrong_object",
        )
...
def _rewrite_report(args, verdict: str) -> str:
...
def run(args) -> str:
    ...
    return _rewrite_report(args, verdict)
...
def main() -> None:
    ...
    parser.add_argument("--max_occluder_displacement", type=float, default=0.002)
    ...
    parser.add_argument("--branch_grasp_prefix_on_contact", action="store_true")
    parser.add_argument("--prefix_grasp_seat_steps", type=int, default=0)
    parser.add_argument("--prefix_lift_max_position_command", type=float, default=None)
