...
def _descend_until_support_contact(
    ...
    """Lower the held target until MuJoCo reports real target-support contact."""
...
def _hold_until_stable_support_contact(
    ...
    """Require persistent plate support and low bowl speed before opening."""
    stable_steps = 0
...
def _confirm_released_on_support(
    ...
    """Keep the EEF still until the released bowl is stably supported."""
    stable_steps = 0
    ...
    for _ in range(args.post_release_support_max_steps):
        ...
        displacement = float(np.linalg.norm(_body_pos(env, TARGET) - release_target_pos))
        max_displacement = max(max_displacement, displacement)
        released = not bool(oracle._metrics(env)["gripper_contact"])
        supported = _target_support_contact(env)
        slow = _body_linear_speed(env, TARGET) <= args.max_post_release_linear_speed
        if (
            released
            and supported
            and slow
...
def _run_episode(
    ...
    if require_support_contact:
        # Concave bowl and rimmed-plate meshes have coarse AABBs whose inferred
        # vertical gap can be wrong by roughly 10 cm.  Use the source/support
        # layer only to stage above the plate; real MuJoCo contact determines
        # the final release height below.
    ...
    if (
        failure is None
        and max_bowl_lift_m > args.max_safe_lift_height
    ):
    ...
    try:
        for idx, state in enumerate(states):
            ...
            print(
                ...
                f"max_lift={row['max_bowl_lift_m']:.4f}m "
                f"support_before_release={row['pre_release_support_contact']} "
                f"released_supported={row['released_before_retreat'] and row['post_release_support_contact']} "
